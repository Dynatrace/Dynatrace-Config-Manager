...
def migrate_settings_2_0():
    ...
    entity_filter = flask_utils.get_arg_json(
        "entity_filter", process_utils.ALL_BASIC_ENTITY_LIST
    )
    ...
    terraform_parallelism = flask_utils.get_arg_int(
    ...
    run_info = process_utils.get_run_info(
        ...
        context_params,
        entity_filter,
        use_environment_cache=use_environment_cache,
        forced_entity_id_main=forced_entity_id_main,
        forced_entity_id_target=forced_entity_id_target,
        forced_schema_id=forced_schema_id,
        forced_key_id=forced_key_id,
        forced_keep_action_only=forced_keep_action_only,
        preemptive_config_copy=preemptive_config_copy,
        enable_dashboards=enable_dashboards,
        enable_omit_destroy=enable_omit_destroy,
        terraform_parallelism=terraform_parallelism,
        enable_ultra_parallel=enable_ultra_parallel,
        action_id=action_id,
    )
...
@blueprint_route_migrate_v2.route("/one_topology_replacements_check", methods=["GET"])
@cross_origin(origin="*")
def one_topology_replacements_check():
    tenant_key_main = flask_utils.get_arg("tenant_key_main", "0")
    tenant_key_target = flask_utils.get_arg("tenant_key_target", "0")
    ...
    def call_process():
        return monaco_cli_match.get_one_topology_replacements_info(run_info)
