...
def get_cached_data(sub_file, file_extension=".json", file_expected=True):
    ...
    try:
        ...
        if file_expected:
            dirs.print_path_too_long_message_cond(sub_file)
            raise e
...
class LoadMatchedEntities:
    ...
    def analyze(self, entities_data):
        ...
        if (
            "matchKey" in entities_data
            and "source" in entities_data["matchKey"]
            and "target" in entities_data["matchKey"]
        ):
        ...
        if "matches" in entities_data:
            for target_id, source_id in entities_data["matches"].items():
                ...
                self.results["matched_entities"][entity_type][target_id] = {
                    "match_entity_list": {
                        source_id: {"top_match": True, "only_top_match": True}
                    },
...
class LoadEntitiesMetadata:
    ...
    def analyze(self, entities_metadata):
        ...
        if (
            entities_metadata["configs"][0]
            and "type" in entities_metadata["configs"][0]
        ):
            pass
        else:
            return
        ...
        if "entitiesType" in entities_metadata["configs"][0]["type"]["entities"]:
            entity_type = entities_metadata["configs"][0]["type"]["entities"][
                "entitiesType"
            ]
        else:
            return
        ...
        if (
            "from" in entities_metadata["configs"][0]["type"]["entities"]
