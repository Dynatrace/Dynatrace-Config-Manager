...
type GetStubData func(idx int) (api.Stub, []byte, bool, error)

func (me *Folder) SaveAllCallback(numItems int, getStubData GetStubData) error {
	me.mu.Lock()
	defer me.mu.Unlock()
	...
	for idx := 0; idx < numItems; idx++ {
		stub, data, isSkipped, err := getStubData(idx)
		if err != nil {
			return err
		}
